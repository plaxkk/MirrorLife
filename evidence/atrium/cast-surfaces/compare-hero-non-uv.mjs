  ...
  for(const suffix of ['.glb','-mobile.glb']){
   ...
   for(const prefix of ['/tmp/cast-surfaces/hero-control', '/tmp/cast-surfaces/hero-regression']){
    ...
    const meshes={};g.scene.traverse(o=>{if(!o.isMesh)return;const geom=o.geometry;
     const names=Object.keys(geom.attributes).filter(n=>!n.startsWith("uv")).sort();
     ...
     meshes[o.name]={names,rows:rows.sort(),tri:tri.sort()};
    });pair.push(meshes);
   }
   const meshes=Object.keys(pair[0]).map(name=>({name,same:JSON.stringify(pair[0][name])===JSON.stringify(pair[1][name]),vertices:pair[0][name].rows.length,triangles:pair[0][name].tri.length}));results.push({suffix,meshes});
  }
  decoder.dispose();return results;
 });await fs.writeFile('evidence/atrium/cast-surfaces/hero-control-non-uv-comparison.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
