...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  for(const suffix of ['.glb','-mobile.glb']){
   const pair=[];
   for(const prefix of ['', '/tmp/cast-surfaces/hero-regression']){
 ...
 });await fs.writeFile('evidence/atrium/cast-surfaces/hero-decoded-regression.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
