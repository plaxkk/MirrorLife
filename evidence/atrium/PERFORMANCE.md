...
构建指纹：`14a1400c550547f41974e2c6f2bc8ddc1abab0ab7bb25cd5851e2fcd2e61655c`。本文件从同候选原始 JSON 生成，不以构建成功代替玩家体验或美术验收。
...
- 桌面采样时间：2026-09-15T14:58:23.399Z；移动采样时间：2026-09-15T14:59:12.386Z。本机 production preview，可见 Chrome，进入后预热，再实际拖动环绕 30 多秒。采样与 Blender、录像顺序执行。
...
| P99 ms | 33.4 | 17.6 |
| CPU 脚本 P95 ms | 4 | 2.1 |
| CPU 渲染提交 P95 ms | 6.1 | 2.6 |
| GPU 样本数 | 190 | 194 |
| GPU P50 ms | 9.81 | 6.01 |
| GPU P95 ms | 18.87 | 7.67 |
| 峰值 draw calls | 99 | 81 |
| 峰值 triangles | 488768 | 322608 |
| 峰值 geometries | 80 | 76 |
| JS heap bytes | 61910179 | 50489670 |
| 导航至就绪 ms | 4345.2 | 4068.3 |
| 模块启动至就绪 ms | 3780.1 | 3665.4 |
| 点击至活动帧 ms | 178.5 | 97.2 |
| 镜头查询索引 triangles | 170127 | 102944 |
| 镜头索引构建 ms | 191.6 | 162.2 |
| 资源 transferBytes | 6819597 | 5767485 |
| 资源 decodedBytes | 11467642 | 10415530 |
...
资产清单 84 项，public 15243412 bytes，source/master 101719753 bytes。Resource Timing 合计不含导航文档，不等于整个发布体积。镜头索引复用已解码 GLB，不额外下载模型；其 CPU 和 native 内存成本仍需考虑。

## 连续流程与边界

录像 107.8 秒 / 1617 帧 / 38108792 bytes，全片解码 exit 0。SHA-256：`664b216d682256be2278d123130a34520d2783f501bd77c2ecb9135cb68a9734`。录像使用本机 Chrome headless 实际 WebGL，按真实时间编码 15fps，不用于替代独立性能采样，也不代表人类 3–5 分钟阅读决策时间。

回游峰值 105 calls / 476000 triangles / 77 geometries。步骤、异常和前后指纹见 acceptance.json，各用例日志在 validation/。方向、家具和触摸专项见 controls/report.json。视频画幅、内容与自然度仍需要独立视觉检查。
