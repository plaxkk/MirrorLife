...
- 背面、侧面、顶部和底部由独立参考图确定，并具有连续结构，不接受仅凭单图猜测的通用背面。
...
2. **补齐一致多视图**
   - 必须生成正、背、左、右、顶、底和等距七张同一物品的独立参考图。
...
4. **人工修模**
   ...
   npm run place:interior-3d -- \
     ...
     --reference-views front,back,left,right,top,bottom,isometric \
     --reference-files front.png,back.png,left.png,right.png,top.png,bottom.png,isometric.png \
     --review-report desk-review.json \
     --geometry-audit desk-geometry.json \
...
6. **Web LOD**
   - 单个主要陈设不超过 80k 三角面。
   - 单个 GLB 不超过 12 MB，建议 2-6 MB。
   - 使用 Meshopt/Draco 和 KTX2 压缩，并保留未压缩母版。

7. **验收**
   - 用与参考图一致的等距相机渲染 turntable 首帧。
   - 七个标准视图的轮廓 IoU 不低于 0.92，主色相似度不低于 0.88。
   - 按 `requiredParts` 检查每个语义部件的形状、位置和材质。
   - 检查至少 12 帧 360 度转台、法线、接地、透明、穿模和移动端加载。
...
dist/interior-3d-work/
  fidelity-packets/<slot>/    # 七视图、部件清单、转台与交付模板
...
当前运行版中的 AI 单图重建与程序化模型都属于玩法开发资产。只有来源标记为 `tripo-multiview`、`hunyuan-multiview` 或 `blender-manual`，并通过七视图、逐部件、360 转台和拓扑审计的模型，才能视为最终美术资产。
