...
|---|---:|---:|---:|---:|---:|
| 桌面 | 1 | 1,331.0ms | 2,500ms | 1.3ms | 800ms |
| 桌面 | 2 | 316.6ms | 2,500ms | 1.0ms | 800ms |
| 移动 | 1 | 402.7ms | 4,000ms | 35.2ms | 1,200ms |
| 移动 | 2 | 398.8ms | 4,000ms | 19.4ms | 1,200ms |

生产构建本地预览的一体化门禁结果：

| 指标 | 桌面 | 移动端 |
|---|---:|---:|
| 地图首屏传输 | 1,381,677B | 1,381,677B |
| 地图拖动 P95 | 7.1ms | 1.7ms |
| 区块生成 P95 | 4.6ms | 3.0ms |
| 室内冷路径 | 366.7ms | 292.4ms |
| 室内热路径 | 1.0ms | 32.4ms |
...
- 桌面 0°/90°/180°/270° 分别为
  `133/326,799`、`136/337,203`、`136/374,003`、`135/362,959`
  （draw calls/triangles），均不超过 `180/450,000`。
- 移动端 0°/90°/180°/270° 分别为
  `90/249,475`、`90/249,475`、`91/249,865`、`90/249,475`，
  均不超过 `110/250,000`。反向见证墙保留相同构图和配色，仅把移动端
  子像素圆角改为低面 joinery，并合并为一个顶点色批次。
- 26 个区域、10 种原型物理门禁通过；人物真实移动 `3.33m`、旋转
