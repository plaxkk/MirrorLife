...
四角色 GLB 材质 70 个（player 17 / listener 17 / facilitator 19 / mediator 17）
```

材质数基本等于 draw call 数。两根未达标的轴都需要**加实体几何**才能补，而余量只有 3 个。不先解决这个，后面每一步都会撞预算。

**验收条件**

| 指标 | 当前 | 目标 |
|---|---:|---:|
| `drawCallsByLayer.actors` | 99 | **≤ 50** |
| opening 总 draw call | 177 | **≤ 140** |
| 七轴达标数 | 5/7 | **不低于 5/7** |
| 硬约束 | 全绿 | 全绿 |

**做法**：把每个角色的 17–19 个材质图集化到 ≤ 6 个（皮肤 / 头发 / 上装 / 下装 / 鞋 / 配件）。UV 重排 + 贴图打包在 `scripts/blender-build-civic-characters.py` 导出前完成。
