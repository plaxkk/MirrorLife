...
运行时会懒加载 GLB、校验头部与四肢枢轴，加载完成前不揭示最终房间；异常时才退回程序化角色。四个角色已经进入真实灯光、遮挡、阴影、碰撞、WASD 移动、跳跃和环绕镜头系统。眼睛使用独立 `EyePivot_-1 / EyePivot_1` 枢轴执行眨眼，眉毛拥有独立表情枢轴，嘴部提供可切换的 `MouthClosedPivot / MouthOpenPivot`，说话时不再只缩放一条嘴线；表情运行时按 Three.js 的 Y-up 坐标驱动，避免把 Blender 的竖直动作误写到深度轴。2D 头像中的高识别特征不再只停留在配色：眼白、虹膜、瞳孔与高光按头像比例重建为贴合头部的曲面几何，粗线感由可眨眼的 `UpperLid` 实现，引导者与调解者另有角色化外眼睫；耳廓、眉弓、腮红和非对称嘴角也在真实灯光与遮挡中成立，而不是相机朝向的肖像贴片。手臂由肩部与肘部、腿部由髋部与膝部两级关节驱动，鞋、裤脚和褶皱均跟随正确的下肢段，因此倾听、记录、思考、行走和落地不再共用僵直四肢。角色发型不再依赖重复胶囊，而是用低面数、多截面、渐细的弯曲发束建立正面、侧面和背面轮廓；引导者、调解者和倾听者也拥有可从游戏镜头直接读出的笔记、思考和开放手势。角色材质在同一渲染通道内加入克制的插画墨线边缘，因此不会用翻倍网格换轮廓。移动端保留肘膝动作，并把细小五官与手指细节合并进低批次 LOD；桌面端保留独立眼睑、眉毛和开合嘴状态。它们是可玩的共享骨架资产基线，但仍未达到最终的面部 blend shape、完整手部拓扑、动态布料和手工动画片段质量。
