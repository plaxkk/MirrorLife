...
运行时会懒加载 GLB、校验头部与四肢枢轴，加载完成前不揭示最终房间；异常时才退回程序化角色。四个角色已经进入真实灯光、遮挡、阴影、碰撞、WASD 移动、跳跃和环绕镜头系统。眼睛使用独立 `EyePivot_-1 / EyePivot_1` 枢轴执行眨眼，眉毛和嘴部拥有独立表情枢轴；表情运行时按 Three.js 的 Y-up 坐标驱动，避免把 Blender 的竖直动作误写到深度轴。手臂由肩部与肘部、腿部由髋部与膝部两级关节驱动，鞋、裤脚和褶皱均跟随正确的下肢段，因此倾听、记录、思考、行走和落地不再共用僵直四肢。角色材质在同一渲染通道内加入克制的插画墨线边缘，因此不会用翻倍网格换轮廓。移动端保留肘膝动作，但把亚像素级五官与手指细节合并为低批次 LOD。它们是可玩的共享骨架资产基线，但仍未达到最终的面部 blend shape、完整手部拓扑、布料褶皱和手工动作质量。
