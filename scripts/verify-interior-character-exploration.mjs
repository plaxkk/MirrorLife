...
try {
  ...
  assert(opening.actors.every((actor) => actor.faceMode === "uv-hybrid"), "civic scene did not use the production head-UV/volumetric-eye facial contract");
  ...
  assert(opening.actors.every((actor) => actor.facial?.integration === "mirrorlife-civic-face-uv-hybrid-v1"), "civic actors did not preserve the production head-UV/volumetric-eye facial contract");
  assert(opening.actors.every((actor) => actor.facial?.morphCount === 4), "civic UV head morph set is incomplete");
  ...
  assert(opening.actors.every((actor) => actor.eyes?.blinkAxis === "z"), "civic volumetric eyes lost the authored blink axis");
  ...
  assert(Number(opening.actors.find((actor) => actor.assetRole === "player")?.facial?.smile || 0) > 0.2, "player UV head did not receive the authored warm-smile morph");
